/* script.js - v1.9: Final Stage, Mission Impossible & You Win! */
...
let isPlaying = false;
let isDying = false; 
let isVictory = false; // Stato di vittoria finale
...
function resizeCanvases() {
    ...
    if(isVictory) drawVictory(); // Ridisegna vittoria se resize
}
...
function initGame(lvl, resetLives = true){
    ...
    // --- SETUP NEMICI (MULTIPLE SPIDERS PROGRESSION) ---
    qixList = [];
    let numSpiders = 1;
    
    if (level >= 8) numSpiders = 4;      // MISSION IMPOSSIBLE
    else if (level >= 7) numSpiders = 3; // FINAL STAGE
    else if (level >= 5) numSpiders = 2; // Mid game

    for(let i=0; i<numSpiders; i++) {
        let startX = Math.floor(W * 0.3) + (i * 20);
        let startY = Math.floor(H * 0.3) + (i * 10);
        // Evita spawn fuori griglia se troppi ragni
...
function draw() {
    ...
    if (isPlaying) {
        ...
        for(let i = floatingTexts.length - 1; i >= 0; i--){
            ...
            entCtx.save();
            let color = ft.color || 'white'; // Default white
            ...
            // Muovi verso l'alto leggermente
...
// Funzione speciale per disegnare la vittoria
function drawVictory() {
    entCtx.clearRect(0,0,entityCanvas.width,entityCanvas.height);
    
    // Disegna testo YOU WIN
    entCtx.save();
    entCtx.fillStyle = '#00ff00'; // Verde Vittoria
    entCtx.shadowColor = '#00ff00';
    entCtx.shadowBlur = 30;
    
    // Scritta grande al centro
...
function closeStixAndFill(){
    ...
    // Per il flood fill, dobbiamo partire da TUTTI i ragni per proteggere le aree vuote
    for(let q of qixList) {
...
function resetAfterDeath(){
    ...
    if(lives <= 0){
        ...
        Swal.fire({
            ...
            gameoverSound.pause();
            gameoverSound.currentTime = 0;
            if(isMusicOn) { bgMusic.currentTime = 0; bgMusic.play(); }
            initGame(1, true); 
        });
    } else {
        stixList = []; 
        ...
        qixList = []; // Pulisci e ricrea per sicurezza posizioni start
        for(let i=0; i<numSpiders; i++) {
            let startX = Math.floor(W * 0.3) + (i * 20);
            let startY = Math.floor(H * 0.3) + (i * 10);
            if(startX >= W-2) startX = W-10;
...
function winLevel() {
    ...
    if (level >= MAX_LEVEL) {
        // VITTORIA FINALE
        isVictory = true;
        drawVictory(); // Disegna scritta YOU WIN
        // Non mostrare il bottone next level
    } else {
        // Livello Normale
...
function gameLoop(now){
    ...
    if(!isVictory) draw();
    // Se è vittoria, non ridisegniamo continuamente il loop di gioco standard
